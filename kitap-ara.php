<?php
include 'veritabani.php'; // Veritabanı bağlantısı ($conn) burada tanımlanmış olmalı
...
if ($query_execute) {
    $base_query_from_joins = "FROM kitaplar 
    ...
    $count_query_sql = "SELECT COUNT(DISTINCT kitaplar.kitap_id) as total_count $base_query_from_joins $final_where_clause";
    ...
    if ($total_items > 0) {
        ...
        $query = "$main_query_select $base_query_from_joins $final_where_clause $main_query_order_limit";
...
if (!empty($search_input)) {
    $pagination_query_params['search'] = $search_input;
}
?>
...
<head>
    ...
    <title>Kütüphane Otomasyonu - Kitap Ara</title>
    ...
    <nav aria-label="Sayfa navigasyonu" class="mt-4">
        <ul class="pagination justify-content-center">
            <?php if ($current_page > 1): ?>
                <li class="page-item">
                    <a class="page-link" href="?page=<?= $current_page - 1 ?>&amp;<?= http_build_query($pagination_query_params) ?>" aria-label="Önceki">
            ...
            if ($start_page_loop > 1) {
                echo '<li class="page-item"><a class="page-link" href="?page=1&amp;' . http_build_query($pagination_query_params) . '">1</a></li>';
            ...
            for ($i = $start_page_loop; $i <= $end_page_loop; $i++): ?>
                <li class="page-item <?= ($i == $current_page) ? 'active' : '' ?>">
                    <a class="page-link" href="?page=<?= $i ?>&amp;<?= http_build_query($pagination_query_params) ?>"><?= $i ?></a>
                </li>
            <?php endfor;
            ...
            if ($end_page_loop < $total_pages) {
                ...
                echo '<li class="page-item"><a class="page-link" href="?page=' . $total_pages . '&amp;' . http_build_query($pagination_query_params) . '">' . $total_pages . '</a></li>';
            }
            ?>

            <?php if ($current_page < $total_pages): ?>
                <li class="page-item">
                    <a class="page-link" href="?page=<?= $current_page + 1 ?>&amp;<?= http_build_query($pagination_query_params) ?>" aria-label="Sonraki">
